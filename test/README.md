...
**`math_verification.js` (892 checks) now covers that gap** and goes wider than the original did — every numerical primitive in the app checked against an independently-derived value.
...
- `math_verification.js` — 892 numerical checks against independently-derived reference values; needs no DOM, runs straight against the engine source (see "The lost coverage" above for scope and rules)
