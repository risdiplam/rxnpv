...
function ReferenceSheet({ activeCase }) {
  ...
  const guideTab = () => h("div", null,
    ...
    card([
      label("Tools — six workbenches"),
      h("div", { style: { fontFamily: "var(--sans)", fontSize: 12, color: "var(--ink-2)", lineHeight: 1.6, maxWidth: PROSE, marginBottom: 12 } },
        "Tools is organised by the question you are asking, not by how each tool works. Six workbenches, each holding two to four tools that are usually used together and can still be used alone: ",
        h("b", null, "Trial"), " (what is this trial, what can it prove, what did it report), ",
        h("b", null, "Science"), " (is the target real, what has been published), ",
        h("b", null, "Company"), " (can it reach its next catalyst, who is buying or selling it), ",
        h("b", null, "Commercial"), " (it is already selling — is the launch tracking, when does it end), ",
        h("b", null, "Valuation"), " (what is the case worth, what is it most sensitive to), and ",
        h("b", null, "Benchmarks"), " (what comparable deals and drugs actually did). Picking a workbench lands you on its first tool; the rest are one click away on the row below."),
      feature("Trial Decoder", "Paste an NCT number and get the trial's architecture in plain English — allocation, masking, comparator, arms, primary endpoint — plus what that design can and cannot establish, and design red flags. Where CT.gov is silent it says “not stated” rather than assuming a default. Once a trial has reported, the same page gains what the endpoints returned (with the sponsor's own effect estimate, interval and p-value, and whether the comparison was superiority or non-inferiority), who finished and who left by arm, and the adverse events — deaths separated from every other reason for leaving, because in a serious indication most of an arm can be “did not complete — death” and folding that into a dropout rate produces a large, confident, meaningless number. Underneath, a search for what has actually been published about that trial."),
      feature("Asset Program", "Every registered trial for one drug at once, by phase, with the trials that stopped and the reason each gave. Carries an evidence-base checklist — how many are randomised, blinded, controlled, have posted results, and how large the biggest one is — deliberately as counts with their own denominators and never as a composite score."),
      feature("Target Dossier", "Human genetic support for a target, disease associations split by whether the evidence is genetic or merely inferred, and what drugs already exist against it. Deliberately not wired into the PoS model: an aggregate association score presented as a probability input would be exactly the false precision this app avoids."),
      feature("Literature", "Searches Europe PMC, which despite the name indexes MEDLINE/PubMed in full plus open-access text and preprints. What it adds over a plain web search is the thing a search engine cannot tell you: what kind of paper each result is. A primary randomised trial report, a meta-analysis, a narrative review, a conference abstract and an unreviewed preprint arrive already separated — and the composition of a result set (“4 primary papers, 15 reviews”) describes an evidence base in one line."),
      feature("Launch & Actuals", "For a drug that is already selling. The launch tracker reads CMS's Medicare Part D and Part B drug spending, which publishes quarterly and runs about one quarter behind, as a live uptake proxy — with analogs indexed to each drug's first Medicare year. It is not revenue: Medicare only, gross of rebates, and small counts suppressed, so read the shape and direction rather than the level. Actual vs modelled puts reported product revenue next to this case's own projection. Both refuse to compare a partial period to a full one — three quarters against a modelled year shows a 25% miss on a drug that is exactly on plan."),
    ...
    card([
      label("Getting results out — export, “+ Report”, and the PDF report"),
      feature("Export any section", "Every section in every view — each Workspace card, each tool, each Simulation panel, each card on this sheet, Portfolio — has an Export row along its bottom edge. PNG and PDF save the WHOLE section: its title, headline figures, the inputs as you set them, tables, charts and any note you have opened, at full height even when it is taller than the window, with a footer saying where it came from. PDF is vector, with selectable text. SVG saves just a chart, editable in design tools; a section with several charts lets you pick which."),
      feature("“+ Report”", "Adds that section to a case's PDF report — the case shown next to the button when you have more than one. It then says where it went, with a link straight to the report. Sections the report already builds from your model (revenue chart, cash flow, bridge, sum-of-the-parts, scenarios) are switched on or off rather than copied, so they never go stale. Anything else — a trial decoded, a statistics result, a comp lookup, a card from this sheet — is stored as it looked when you added it."),
      feature("PDF bundle — combine anything, no case needed", "Every export row also has “+ Bundle”. It collects that section or chart into your bundle — Bundle in the top bar shows how many you have. Collect a Simulation panel with its inputs and result, just its chart, a Tools lookup, a Reference Sheet table, from as many places as you like; then in Bundle choose which to include and their order, and export them merged into one PDF (optionally one item per page) or as separate PDFs saved into a folder you pick once. The bundle is kept until you clear it."),
      feature("PDF report", "Open it with Generate Report (the badge counts what you have added). Under Sections ▼, tick exactly what goes in: the built-in sections, with Everything / Summary only / Charts only presets, and every added section individually, each with ↑ ↓ to set its order and ✕ to remove it. Added sections follow the report's light or dark setting, whichever mode they were added in. Sections that don't apply to a case stay hidden even when ticked — Sum-of-the-parts needs more than one program, and the year-by-year charts need DCF rather than Simple Multiple.")
    ]),
    card([
