...
function ReferenceSheet({ activeCase }) {
  ...
  const guideTab = () => h("div", null,
    ...
    card([
      ...
      feature("PK/PD", "Forward-looking. Projects a concentration-time profile from published PK parameters (dose, absorption/elimination rates, volume of distribution), optionally bridges to receptor occupancy, then maps exposure through a dose-response (Emax) curve — a forward projection from parameters you supply, not a fit to patient data. A collapsed note on the tab explains where each parameter is actually published (FDA label, Phase 1 PK paper) and how to derive the ones that usually aren't stated directly. Carries a standalone Receptor Occupancy Calculator underneath for when you already have a concentration in hand and don't need the full dosing simulation to get to a % occupied.")
    ]),
    card([
      label("Custom comps"),
